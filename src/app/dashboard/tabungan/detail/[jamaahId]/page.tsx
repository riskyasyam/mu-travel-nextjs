        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="jumlahSetoran" className="text-right">
                Jumlah
              </Label>
...
// --- Komponen Utama Halaman Detail Tabungan ---
export default function DetailTabunganPage() {
  const params = useParams();
  const jamaahId = params?.jamaahId as string;
    ...
    <div className="p-8">
      ...
      <div className="mb-6 bg-green-100 border-l-4 border-green-500 text-green-800 p-4 rounded-md">
        <p className="font-bold">
          Total Saldo Saat Ini: Rp {totalTabungan.toLocaleString('id-ID')}
        </p>
      </div>
            ...
            {jamaah.tabungan.length > 0 ? (
              jamaah.tabungan.map((setoran) => (
                <TableRow key={setoran.id}>
                  <TableCell>
                    {new Date(setoran.tanggalSetoran).toLocaleString('id-ID', {
                      dateStyle: 'full',
                      timeStyle: 'short',
                    })}
                  </TableCell>
                  <TableCell className="font-medium">
                    Rp {setoran.jumlahSetoran.toLocaleString('id-ID')}
                  </TableCell>
